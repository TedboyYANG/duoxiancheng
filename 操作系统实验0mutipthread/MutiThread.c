...
int main(int argc, char** argv)
{
	...
	for (q = 0; q < p; q++)
	{
		...
		int errorCode = pthread_create(&single, NULL, ThreadFuc, (void*)& q);
		if (errorCode)
		{
			printf("create pthread faild code:%d\n", errorCode);
			continue;
		}
		pthread_join(pthreads[q], &retval);
	}
	return 0;
}
...
void* MutiThread(void* t)
{
	int q = *(int*)(void*)t;
	printf("ID:%d\n", q);
	char order_date[] = " ";//日期关键字
	char ship_date[] = " ";//日期关键字
	int k, j, l = 0, m = 0, i = 0;//循环变量
